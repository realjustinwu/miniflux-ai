...
def miniflux_ai_news():
    ...
    try:
        ...
        logger.error("Error reading ai_news.json: %s" % e)
        ai_news = ""
    ...
    try:
        ...
        logger.error("Error clearing ai_news.json: %s" % e)
    ...
    if ai_news:
        ...
        fe.title(
            f"{'Morning' if datetime.today().hour < 12 else 'Nightly'} Newsᴬᴵ for you - {time.strftime('%Y-%m-%d')}"
        )
